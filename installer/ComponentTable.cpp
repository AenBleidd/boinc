...
ComponentTable::ComponentTable(const std::vector<Directory>& directories) {
    std::unordered_set<std::string> components_map;
    for (const auto& directory : directories) {
        for (const auto& component : directory.getComponents()) {
            if (components_map.find(component.get_component()) != components_map.end()) {
                continue;
            }
            components.push_back(component);
            components_map.insert(component.get_component());            
        }
    }
}

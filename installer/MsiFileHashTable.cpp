...
MsiFileHashTable::MsiFileHashTable(const std::vector<File>& files) {
    for (const auto& file : files) {
        ...
        if (MsiGetFileHash(file.getFilepath().string().c_str(), 0, &hashInfo) != ERROR_SUCCESS) {
            std::cerr << "Failed to get file hash for " << file.getFilepath() << std::endl;
            continue;
        }
        msiFileHashes.emplace_back(file.getFileId(), hashInfo.dwData[0], hashInfo.dwData[1], hashInfo.dwData[2], hashInfo.dwData[3]);
    }
}
